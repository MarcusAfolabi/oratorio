        ...
        <div class="flex justify-between items-center relative md:mb-4 mb-3">
            <div class="flex-1">
                <h5 class="text-1xl font-semibold">
                    <a uk-tooltip="Add" href="#otherPost" uk-toggle><i class="icon-material-outline-add"></i> Job content </a>
                </h5>
            </div>
        </div>
                                ...
                                <tbody>
                                    ...
                                    <tr class="border-b">
                                        ...
                                        <th class="text-sm text-blue-900 font-light px-6 py-4 whitespace-nowrap">
                                            @if($job->images) @foreach ($job->images as $image)<a href="{{ asset('storage/' . $image->path) }}"> <i class="icon-material-outline-launch"></i>Open </a>@endforeach @endif

                                        </th>
                                        <th class="text-sm text-green-900 font-light px-6 py-4 whitespace-nowrap">
                                            {{ $job->views }}
                                        </th>
                                        <td class="text-sm text-green-900 font-light px-6 py-4 whitespace-nowrap"> <a href="{{ route('post.edit', $job) }}"> <span class="icon-feather-edit "></span></a></td>
                                        <td class="text-sm text-green-900 font-light px-6 py-4 whitespace-nowrap">
                                            <form action="{{ route('post.destroy', $job) }}" method="POST">
                                                @csrf
                                                @method('delete')
